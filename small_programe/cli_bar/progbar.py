...
class ProgBar(Prog):
    ...
    def __init__(
        ...
        super().__init__(
            iterations, track_time, stream, title, monitor, update_interval
        )
        # 进度条宽度（横向长度）
    ...
    def _print_labels(self):
        self._stream_out(f"0% {' ' * (self.bar_width - 6)} 100%\n")
        self._stream_flush()

    def _print_progress_bar(self, progress):
        # 打印进度条
        remaining = self.bar_width - progress
        self._stream_out(f"[{self.bar_char * int(progress)}{' ' * int(remaining)}]")
        self._stream_flush()
